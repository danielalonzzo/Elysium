...
Cada prospecto tiene su carpeta, y dentro una carpeta `prototipo/` que concentra
todo el material. Es la carpeta que después se conecta al IDE para que el
asistente de IA la lea.
...
└── prototipo/
    ...
    │   └── notas-comerciales.md transcripción de llamadas y correos
    ...
    └── prototipo/               ← Fase 3 · el sitio navegable
        ├── index.html
        ├── css/  ·  js/  ·  images/
        └── elysium-core/        (submódulo; ver §4.2 del estándar)
```

**El material en bruto nunca se borra.** Cuando el proyecto pasa a producción,
la carpeta `prototipo/` se conserva como memoria del proyecto: es la única
prueba de qué sabíamos y qué prometimos antes de firmar.
...
Trabajo **manual** de un colaborador. Consiste en llenar `00-recoleccion/` con
...
Se abre la carpeta `<NombreDelProyecto>/prototipo/` en el IDE, de modo que el
asistente tenga acceso de lectura a todo `00-recoleccion/`, y se le entrega el
prompt de §IV.2 junto con este documento.
...
- Cada afirmación se apoya en material de `00-recoleccion/`. Si algo no consta,
...
`02-informe-aprobado.md`, que a partir de ese momento es **la única fuente**
...
> **Compuerta 1.** Sin `02-informe-aprobado.md` no se escribe una sola línea de
...
Se genera el prototipo directamente a partir de `02-informe-aprobado.md`, con el
...
`03-informe-mejoras.md`, separando las propuestas en tres bloques y ordenándolas
por impacto:
...
con el alcance del plan contratado, y se publica en el dominio del cliente.
...
- Leer `00-recoleccion/` completo antes de redactar el informe preliminar, y
...
- Generar el prototipo **únicamente** a partir de `02-informe-aprobado.md`,
...
Tiene acceso a la carpeta 00-recoleccion/ de este proyecto. Léala por completo
antes de escribir.
...
- [ ] Revisado y firmado por el equipo como `02-informe-aprobado.md`.
...
*Elysium λ Development & Research · Protocolo interno v1.1.0 · Complementa a
