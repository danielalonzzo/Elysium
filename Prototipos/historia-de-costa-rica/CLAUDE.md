...
carpetas con este nombre, en **dos sitios distintos del disco**:

| Ruta | Qué es | Se edita a mano |
|---|---|---|
| `Elysium λ/Prototipos/historia-de-costa-rica/` | **El código fuente.** Es esta carpeta. Aquí se trabaja. | **Sí** |
| `Elysium λ/Elysium/historia-de-costa-rica/` | **El resultado compilado**, que es lo que se sirve en `elysiumdr.eu/historia-de-costa-rica/` | **Nunca** |
...
lo que se apartó fue el código fuente, a `Prototipos/`, que no es un directorio
publicable.
...
⚠️ **Esta carpeta (el código fuente) no está bajo control de versiones.** Al
salir de Elysium salió también de git. Su única copia es iCloud.
...
```bash
cd "../../Elysium" && ./scripts/publish-historia-de-costa-rica.sh
```

Publicación. **El script vive en Elysium, no aquí**, porque su trabajo termina
escribiendo dentro del sitio. Localiza esta carpeta por su cuenta (en
...
2. Reescribe con `perl` las rutas absolutas que Next **no** toca porque están
   ...
   `/elysium-core`, `/logo.jpg`, `/tienda`). Por eso el código fuente puede
   seguir usando rutas de raíz y funcionar en `npm run dev`.
...
- **Esta carpeta nunca debe acabar dentro de Elysium.** Si el proyecto termina
  copiado dentro de una carpeta publicada, `_comercial/` (los informes internos
  del cliente) se serviría en abierto. Ya pasó una vez. Como red de seguridad,
  las tres listas de despliegue de Elysium — `firebase.json` (`hosting.ignore`),
