...
const LEAF_BG = "/assets/escena/textura-selva.svg";
...
const CATEGORY_CHIPS: { label: string; slug: string; match: string[] }[] = [
  ...
  { label: "Expediciones", slug: "expediciones", match: ["Expediciones"] },
  { label: "Textiles", slug: "textiles", match: ["Textiles"] },
  { label: "Peluches", slug: "peluches", match: ["Peluches"] },
  { label: "Gorras", slug: "gorras", match: ["Gorras"] },
  { label: "Cerámica", slug: "ceramica", match: ["Cerámica"] },
  { label: "Café y cacao", slug: "cafe-y-cacao", match: ["Café y cacao"] },
  { label: "Artesanía", slug: "artesania", match: ["Artesanía"] },
  { label: "Accesorios", slug: "accesorios", match: ["Imanes", "Llaveros", "Botellas", "Mochilas", "Libretas"] },
];
...
export function ShopRedesign({
  ...
  const showExpeditionBanner = activeChip === "todos" && !isFiltered && safePage === 1;
      ...
      <header className="rgx-shop-hero">
        ...
        <div className="rgx-shop-hero-content">
          <p className="rgx-shop-eyebrow">
            ...
            {t("Expediciones y recuerdos", "Expeditions and keepsakes")}
          </p>
          <h1 className="rgx-shop-heading">
            {forcedCategory || forcedTag ? (
              <>{t("Colección", "Collection")}<br /><em>{forcedTag || activeLabel}</em></>
            ) : (
              <>{t("Tienda", "Shop")}<br /><em>Selva y Sal</em></>
            )}
          </h1>
          <p className="rgx-shop-subtitle">
            {filtered.length} {filtered.length === 1 ? t("pieza", "item") : t("piezas")} {t("hechas y guiadas en Costa Rica", "made and guided in Costa Rica")}
          </p>
        </div>
      </header>
          ...
          <div className="rgx-shop-toolbar">
            <label className="rgx-shop-search">
              ...
              <input
                ...
                placeholder={t("Buscar expediciones, textiles, café…", "Search expeditions, apparel, coffee…")}
      ...
      >
        {visible.length > 0 ? (
          <>
            ...
            {/* Banner intercalado: las expediciones guiadas */}
            {showExpeditionBanner && visible.length >= 6 && (
              <Link href="/expediciones/" className="rgx-shop-banner rgx-shop-banner-expo">
                <div className="rgx-shop-banner-content">
                  <span className="rgx-shop-banner-tag">{t("Salidas todo el año", "Departures year-round")}</span>
                  <h3>{t("Expediciones", "Expeditions")}</h3>
                  <p>{t("Once rutas guiadas, de ocho personas como máximo", "Eleven guided routes, eight people at most")}</p>
                  <span className="rgx-shop-banner-cta">{t("Ver las rutas", "See the routes")} <i aria-hidden="true">&#x2197;&#xFE0E;</i></span>
                </div>
                <img
                  src={localAsset("/assets/escena/banner-expediciones.svg")}
                  alt=""
                  loading="lazy"
                />
              </Link>
