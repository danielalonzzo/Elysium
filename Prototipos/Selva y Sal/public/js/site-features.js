/** Selva y Sal · F02, F03, F04 y F09 según la implementación canónica. */
(function () {
  "use strict";

  if (window.__SELVAYSAL_SITE_FEATURES__) return;
  window.__SELVAYSAL_SITE_FEATURES__ = true;
  ...
  function initHeaderContrast() {
    ...
    var dock = document.querySelector(".rg-dock");
    ...
    function sync() {
      ...
      if (header) {
        ...
        for (var i = 0; i < stack.length; i += 1) {
          if (!stack[i].closest("#header") && !stack[i].closest(".rg-dock") && !stack[i].closest("#ely-preloader")) {
            underlying = stack[i];
            break;
          }
        }
      ...
      if (dock) {
        ...
        for (var j = 0; j < stackD.length; j += 1) {
          if (!stackD[j].closest("#header") && !stackD[j].closest(".rg-dock") && !stackD[j].closest("#ely-preloader")) {
    ...
    window.addEventListener("elysium:settings:changed", request);

    // No hay sondeo inicial a propósito. El estado de partida ya viene resuelto
    // desde el servidor en la clase del `<header>`, y forzar aquí una medición
    // la tomaba antes de que la hoja de estilos pintara el fondo crema de la
    // página: la sonda veía entonces el verde profundo del `<body>` y dejaba la
    // píldora en oscuro — logotipo crema sobre crema — hasta el primer scroll.
    // La sonda sólo hace falta en las transiciones reales (en Inicio, cuando la
    // píldora cruza de la portada oscura al comercio crema), y esas siempre
    // llegan por scroll o resize, ya con el CSS aplicado.
